        ...
        <div class="row">
                        ...
                        <ul class="nav nav-tabs border-bottom" data-hover="fancyHover">
                            @foreach ($categories as $keys => $category)
                                <li class="nav-item {{ $keys == 0 ? 'active' : '' }}">
                                    <button class="nav-link hover-effect btn-lg {{ $keys == 0 ? 'active' : '' }}"
            ...
            <div class="col-12">
                <div class="tab-content" data-aos="fade-up">
                    @foreach ($categories as $keys => $category)
                        <div class="tab-pane slide show {{ $keys == 0 ? 'show active' : '' }}"
                            id="{{ $category->slug }}">
                            ...
                            <div class="tab-content">
                                                        ...
                                                        <div class="product text-center p-25 radius-md mb-25">
                                                            <figure class="product-img mb-20">
                                                                ...
                                                                <a href="{{ route('user.front.product.details', [getParam(),$product->slug, $product->product_id]) }}"
                                                                    ...
                                                                    class="cart-link btn btn-md btn-primary hover-show"
                                                                    title="{{ $keywords['Add to Cart'] ?? __('Add to Cart') }}"
                                                                    target="_self">{{ $keywords['Add to Cart'] ?? __('Add to Cart')}}
                                                                </a>
                                                                @else
                                                                 @if (!empty(json_decode($product->addons, true)) || !empty(json_decode($product->variations, true)))
                                                                  <a href="{{ route('user.front.product.details', [getParam(),$product->slug, $product->product_id]) }}"
                                                                    ...
                                                                    target="_self">{{ $keywords['Extras'] ?? __('Extras')}}
                                                                </a>
                                                                @endif
                                                                @endif
                                                            ...
                                                            <div class="product-details">
                                                                ...
                                                                <div class="product-price">
                                                                    ...
                                                                    @if ($product->previous_price)
                                                                        <span class="prev-price font-sm"
                                                                            dir="ltr">{{ $userBe->base_currency_symbol_position == 'left' ? $userBe->base_currency_symbol : '' }}{{ convertUtf8($product->previous_price) }}{{ $userBe->base_currency_symbol_position == 'right' ? $userBe->base_currency_symbol : '' }}</span>
                                                                    @endif
                                                                </div>
                                                            </div>
                                                        </div>
                                ...
                                <div class="cta-btn text-center mt-15 mb-25">
                                    <a href="{{ route('user.front.items', [getParam(),'category_id' => $category->id]) }}"
                                        class="btn btn-lg btn-primary" title="{{ $keywords['View All Items'] ??  __('View All Items') }}"
